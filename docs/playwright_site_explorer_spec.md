...
URL検証に失敗した場合、以下の順序で代替URLを探索します。
...
1. 初回URL検証失敗
       ↓
2. 組織名を自動抽出（例: "三井物産環境基金" → "三井物産"）
       ↓
3. 複数クエリでLLM検索（最大3回）
   ├─ クエリ1: "{組織名} 助成金 募集 2026"
   ├─ クエリ2: "{組織名} 補助金 申請"
   └─ クエリ3: "{組織名} 支援 公募"
       ↓
4. LLM検索失敗時 → Playwright深掘り探索
   - Googleで組織サイトを検索
   - サイト内リンクから助成金ページを探索
       ↓
5. 成功 → URL使用 / 全て失敗 → 助成金を除外
```

### 重複回避

- 同一URLが見つかった場合はスキップして次のクエリを試行
